...
func (ps *ProductService) Create(ctx context.Context, p *pb.Product) error {
	id := uuid.NewString()
	p.Id = id
...
func (ps *ProductService) List(ctx context.Context, pageSize int32, filter, orderBy string) ([]*pb.Product, error) {
	return ps.Repo.Delete(ctx, pageSize, filter, orderBy)
}

func (ps *ProductService) Update(ctx context.Context, p *pb.Product) (*pb.Product, error) {
	return ps.Repo.Update(ctx, p)
}
